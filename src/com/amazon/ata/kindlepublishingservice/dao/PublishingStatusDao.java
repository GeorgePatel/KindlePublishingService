...
public class PublishingStatusDao {
    ...
    public List<PublishingStatusItem> getPublishingStatusList(String publishingStatusId) throws PublishingStatusNotFoundException {
        PublishingStatusItem item = new PublishingStatusItem();
        ...
        PaginatedQueryList<PublishingStatusItem> statusList = dynamoDbMapper.query(PublishingStatusItem.class, queryExpression);

        if (statusList.size() == 0) {
            throw new PublishingStatusNotFoundException(String.format("No publishing statuses were found for given publishing status ID: %s", publishingStatusId));
        }
